...
def generate_itinerary_overview(itinerary):
    ...
    """
    prompt = f"""
You are an expert travel planner creating a comprehensive trip overview in markdown format. Generate a detailed, engaging overview using the following information:
...
**Instructions:** Create a markdown-formatted overview (300-400 words) with the following structure:

# 🌟 Your Amazing {itinerary.destination} Adventure

## 📍 **Trip Overview**
Brief welcome paragraph with trip dates, budget, and excitement builder.

## 🎯 **What to Expect**
- **Cultural Highlights:** Major attractions and landmarks
- **Local Experiences:** Authentic activities and hidden gems  
- **Culinary Journey:** Must-try local dishes and dining experiences
- **Budget-Friendly Tips:** Money-saving recommendations

## 🎒 **Perfect for Your Group**
Paragraph tailored to the specific travelers and their interests.

## ✨ **Making Memories**
Closing paragraph about creating unforgettable experiences.

**Requirements:**
- Use markdown headings (#, ##), **bold**, *italic* text
- Include relevant emojis throughout
- Create bullet points with specific recommendations
- Maintain a friendly, enthusiastic tone
- Focus on experiences rather than just listing places
- Customize content based on the provided interests and extras
...
def generate_day_text_gpt(
    ...
    if weather_info.get("error"):
        ...
        weather_str = f"🌤️ {cond}, {tmin}-{tmax} °C"
    ...
    slot_labels = {
        ...
        "lunch": "🍽️ **Lunch**",
        "afternoon": "☀️ **Afternoon Activity**",
        "dinner": "🍷 **Dinner**",
    ...
    for itm in ordered_places:
        role      = itm["role"]
        place     = itm["name"]
        label     = slot_labels.get(role, f"**{role.capitalize()}**")
...
Create **six blocks** with proper markdown formatting. For **each block** deliver **exactly**:

• **Time span** (HH:MM-HH:MM) in bold  
...
- Use markdown formatting (bold, italics, lists where appropriate)
- Keep the order unchanged
- Breakfast/lunch/dinner paragraphs must describe food options
- Friendly tone, English
- End with a **💡 FINAL TIP** section about {destination}

**User preferences:** {extras}
"""
...
def verify_and_update_places(day_text, lat, lng, destination):
    ...
    and append the verified address and a Maps link with proper markdown formatting.
    """
    ...
    for line in lines:
        if "📍" in line:
            ...
            if place_data is None:
                ...
                new_lines.append(f"⚠️ **Warning:** Could not find '{place_candidate}' on Google Places.")
                new_lines.append("")  # Empty line after warning
            else:
                address = place_data.get("formatted_address", "Address not found")
                encoded = quote(address)
                new_lines.append(f"{line}")
                new_lines.append(f"📍 **Address:** {address}")
                new_lines.append(f"🗺️ **[View on Google Maps](https://www.google.com/maps/search/?api=1&query={encoded})**")
